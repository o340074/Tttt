  ...
  return (
    <div className="mx-auto w-full max-w-[1120px] px-4 py-8 md:px-8">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2" role="group" aria-label={t('admin.dashboard.period')}>
          {PRESETS.map((d) => (
            <button
              key={d}
              type="button"
      ...
      {dash.isError ? (
        ...
        <section className="mb-6 grid grid-cols-2 gap-3 md:grid-cols-4">
          <Kpi label={t('admin.dashboard.revenue')} loading={dash.isLoading}
            value={dash.data ? formatMoney(dash.data.revenue, dash.data.currency) : '—'} />
          <Kpi label={t('admin.dashboard.orders')} loading={dash.isLoading}
            value={dash.data ? String(dash.data.orders) : '—'} />
          <Kpi label={t('admin.dashboard.avgOrder')} loading={dash.isLoading}
            value={dash.data ? formatMoney(dash.data.avgOrder, dash.data.currency) : '—'} />
          <Kpi label={t('admin.dashboard.refunds')} loading={dash.isLoading}
            value={dash.data ? formatMoney(dash.data.refunds, dash.data.currency) : '—'} tone="danger" />
        </section>
      )}

      {dash.data && (
        <section className="mb-8 grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
          <OpsTile icon="clock" label={t('admin.dashboard.warmingQueued')} value={dash.data.ops.warmingQueued} />
          <OpsTile icon="spark" label={t('admin.dashboard.warmingInProgress')} value={dash.data.ops.warmingInProgress} />
          <OpsTile icon="eye" label={t('admin.dashboard.warmingQc')} value={dash.data.ops.warmingQc} />
          <OpsTile icon="check" label={t('admin.dashboard.warmingReady')} value={dash.data.ops.warmingReady} />
          <OpsTile icon="alert" label={t('admin.dashboard.warmingOverdue')} value={dash.data.ops.warmingOverdue} tone="danger" />
          <OpsTile icon="mail" label={t('admin.dashboard.openTickets')} value={dash.data.ops.openTickets} />
        </section>
      )}

      <div className="grid gap-6 lg:grid-cols-2">
        <Panel title={t('admin.reports.byCategory')}>
          <DimensionTable rows={sales.data?.byCategory} loading={sales.isLoading}
            currency={sales.data?.currency ?? 'USD'} />
        </Panel>
        <Panel title={t('admin.reports.byGoal')}>
          <DimensionTable rows={sales.data?.byGoal} loading={sales.isLoading}
            currency={sales.data?.currency ?? 'USD'} labelKeyPrefix="admin.goals" />
        </Panel>
        <Panel title={t('admin.reports.topProducts')}>
          <DimensionTable rows={sales.data?.topProducts} loading={sales.isLoading}
            currency={sales.data?.currency ?? 'USD'} />
        </Panel>
        <Panel title={t('admin.reports.fulfillment')}>
          {fulfil.isLoading ? (
            ...
            <dl className="grid grid-cols-2 gap-3 text-sm">
              <Metric label={t('admin.reports.deliveredJobs')} value={String(fulfil.data?.deliveredJobs ?? 0)} />
              <Metric label={t('admin.reports.slaMet')} value={`${fulfil.data?.slaMetPercent ?? 0}%`} />
              <Metric label={t('admin.reports.avgPlan')} value={t('admin.reports.minutes', { n: fulfil.data?.avgPlanMinutes ?? 0 })} />
              <Metric label={t('admin.reports.avgActual')} value={t('admin.reports.minutes', { n: fulfil.data?.avgActualMinutes ?? 0 })} />
              <Metric label={t('admin.reports.refundRate')} value={`${fulfil.data?.refundReplaceRate ?? 0}%`} />
            </dl>
          )}
        </Panel>
      </div>
...
function Kpi({ label, value, loading, tone }: { label: string; value: string; loading?: boolean; tone?: 'danger' }) {
  return (
    <div className="rounded-xl border border-border bg-surface p-4">
      ...
      {loading ? (
        ...
        <div className={`mt-1 font-display text-2xl font-bold tabular-nums ${tone === 'danger' ? 'text-danger' : 'text-text-hi'}`}>
          {value}
        </div>
      )}
    </div>
...
function OpsTile({ icon, label, value, tone }: { icon: 'clock' | 'spark' | 'eye' | 'check' | 'alert' | 'mail'; label: string; value: number; tone?: 'danger' }) {
  return (
    <div className="rounded-xl border border-border bg-surface p-3">
      <Icon name={icon} className={`mb-1 !h-4 !w-4 ${tone === 'danger' && value > 0 ? 'text-danger' : 'text-text-dim'}`} />
      <div className={`font-display text-xl font-bold tabular-nums ${tone === 'danger' && value > 0 ? 'text-danger' : 'text-text-hi'}`}>
        {value}
      </div>
...
function DimensionTable({
  ...
  return (
    <table className="w-full border-collapse text-sm">
      <tbody>
        {rows.map((r) => {
          const label = labelKeyPrefix ? t(`${labelKeyPrefix}.${r.label}`, { defaultValue: r.label }) : r.label;
          return (
