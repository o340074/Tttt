...
2. Если их нет (папка пустая / не та ветка), выполни:
     git fetch --all
     git checkout claude/advault-track-selection-3kek3w   # актуальная база: E0…E11 + M5 release-ops + долги Трека A/B + E12 рефералы
...
подтверждения и/или пост-MVP (E12+). См. готовый промт ниже. Кратко:
  ...
  - Долги Трека B ЗАКРЫТЫ (аллокация discount при частичном возврате E10, grace-период
    ...
    realtime-долга нет.
  - E12+ (пост-MVP): ✅ рефералка ГОТОВА; далее — интеграции Octo/прокси-API,
    полу-автоматизация прогрева, прямая оплата криптой/картой, аналитика/BI, мультивендор.
...
бэкапы, k6), ВСЕ долги Трека B (аллокация discount при частичном возврате E10, grace-
период гарантийного окна E10 через env WARRANTY_GRACE_MINUTES, inline-edit промо E8,
WebSocket realtime-бейдж уведомлений E9), Трек A — fan-out realtime-бейджа через Redis
pub/sub (канал advault:notifications:unread; деградация к локальной доставке + поллинг), и
ПЕРВАЯ ФИЧА ПОСТ-MVP (E12) — РЕФЕРАЛЬНАЯ ПРОГРАММА: код-приглашение (ReferralCode, лениво),
атрибуция на регистрации (?ref=CODE), активация на первой подходящей покупке реферала →
вознаграждение обеим сторонам через ledger (refType=referral, Decimal, идемпотентно,
суммы-снимок), env-конфиг REFERRAL_*, уведомление referral_rewarded, витрина /referrals +
баннер на регистрации, админ-очередь /admin/referrals (REPORTS_STAFF: сводка + отмена
pending), EN/RU. Всё под RBAC/аудитом; lint/typecheck/376 тестов/build зелёные, проверено
вживую на реальном Postgres+Redis (полный цикл: топ-ап→checkout→оба баланса начислены,
2 ledger-credit refType=referral, referral→qualified, уведомление). Есть деплой-помощники:
deploy/install.sh (one-shot HTTP + host-firewall) и deploy/docker-compose.tls.yml +
deploy/Caddyfile (авто-HTTPS через sslip.io).

ВАЖНО — БАЗА КОДА: актуальный код в ветке claude/advault-track-selection-3kek3w
(E0…E11 + M5 release-ops + долги Трека A/B + E12 рефералы; в main ещё не влито).
Если в рабочей папке нет apps/ и packages/:
  git fetch --all
  git checkout claude/advault-track-selection-3kek3w
Новую feature-ветку создавай от неё.

ПЕРЕД РАБОТОЙ прочитай: CLAUDE.md (контекст/конвенции/границы), docs/SESSION-LOG.md
(верхняя запись «Трек B · E12: Реферальная программа» — что сделано), docs/16-development-plan.md
...
ЦЕЛЬ СЕССИИ — ВЫБЕРИ ТРЕК (уточни у пользователя в режиме asking, если не задано):
  B) Пост-MVP E12+ (реферальная программа УЖЕ ГОТОВА; начать со следующей фичи): интеграции
     Octo Browser API + провайдер прокси (авто-провижининг), полу-автоматизация прогрева
     (шаблоны/напоминания/дашборды загрузки операторов), прямая оплата криптой за заказ /
     оплата картой, расширенная аналитика/BI, мультивендорность.
