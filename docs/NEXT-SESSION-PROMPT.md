...
2. Если их нет (папка пустая / не та ветка), выполни:
     git fetch --all
     git checkout claude/advault-e8-admin-panel-s4ia8e   # актуальная база: код E0…E7 + срез E8
...
## ✅ ГОТОВЫЙ ПРОМТ — Сессия E8-cont (остальные модули админки)
...
ЧАСТЬ 1 ЭПИКА E8 (админка/операторка): роли operator/manager (аддитивно на User.role),
admin Orders (таблица+деталь), Warming Kanban + operator workspace (assign/переходы/
чек-лист/захват аккаунта/resolve + bind прокси/Octo из E7), Inventory-UI (прокси/Octo:
список/создание/импорт), admin Stock read. Всё проверено вживую под ролью operator.

ВАЖНО — БАЗА КОДА: актуальный код в ветке claude/advault-e8-admin-panel-s4ia8e
(E0…E7 + срез E8; в main ещё не влито). Если в рабочей папке нет apps/ и packages/:
  git fetch --all
  git checkout claude/advault-e8-admin-panel-s4ia8e
...
платформа фиксирует связки), docs/SESSION-LOG.md (верхняя запись «E8, часть 1» — что
сделано и долги), docs/16-development-plan.md §E8, docs/13-admin-panel-spec.md (ПОЛНАЯ
спека админки), prototype/screens/admin.html (эталон UI), docs/backend/{prisma-schema,
openapi}.md (контракты; там уже admin Orders/Stock/warming/inventory).

ЦЕЛЬ ЭТОЙ СЕССИИ — доделать E8 (модули по docs/13, которых ещё нет). Порядок на твоё
усмотрение, разумный дефолт:
- Catalog & Bundles CRUD (+конструктор комплекта) и Warming plans CRUD (+версии);
- Promo CRUD; Users (список/поиск/блокировка/смена роли — с аудитом и RBAC ELEVATED);
- Finance (сверка ledger, ручной refund из UI: POST /admin/orders/:id/refund с
  идемпотентностью — черновик в openapi), ручная выдача POST /admin/orders/:id/items/
  :itemId/deliver (черновик в openapi);
- Dashboard/Reports (сводки), Staff&roles UI, Settings — по объёму.
...
маршрутах (группы в apps/api/src/auth/roles.ts) + danger-confirm+AuditLog на необратимом;
дизайн по docs/design/* и прототипу (SVG-иконки из спрайта, без эмодзи); i18n EN/RU через
ключи (блок admin.* в locales); состояния loading/empty/error; ГРАНИЦА ПЛАТФОРМЫ —
только бизнес-логистика, без автоматизации прогрева.

РАЗВИЛКИ (asking, не угадывай): вводить ли StaffUser сейчас или дальше жить на User.role;
объём модулей в этой сессии; политика ручного refund/выдачи (частичный/полный, влияние
на warm-задачу). 

КРИТЕРИЙ ПРИЁМКИ: магазином и прогревом можно управлять полностью из админки без доступа
к БД; RBAC/аудит на месте; lint/typecheck/тесты/build зелёные, CI зелёный. DoD — docs/16 §8.

В КОНЦЕ: обнови docs/SESSION-LOG.md (запись + статус E8=готово/в работе + следующий шаг),
при изменении контрактов обнови docs/backend/, commit+push, подготовь промт следующей
сессии (E8-cont или E9) в docs/NEXT-SESSION-PROMPT.md.
```
