...
## ✅ ГОТОВЫЙ ПРОМТ — пост-MVP / доработка (E12+ · остаточный тех-долг)

> Весь MVP (E0…E11), M5 release-ops и долги Трека B готовы по коду. Скопируй блок ниже
> в новую сессию и выбери трек.
...
WebSocket realtime-бейдж уведомлений E9 в том же Nest-процессе с деградацией к поллингу).
Всё под RBAC/аудитом; lint/typecheck/353 теста/build зелёные, проверено вживую на реальном
...
(верхняя запись «Долги Трека B» — что сделано и остаток), docs/16-development-plan.md
...
ЦЕЛЬ СЕССИИ — ВЫБЕРИ ТРЕК (уточни у пользователя в режиме asking, если не задано):
  A) Остаточный тех-долг (небольшой, хорошо очерчен): fan-out realtime-бейджа через
     Redis pub/sub, чтобы WebSocket-пуш доходил до клиентов при нескольких инстансах API
     (сейчас однопроцессный скоуп + поллинг-fallback; docs/17 §7). Вертикальный срез:
     NotificationsRealtimeService публикует/подписывается на Redis-канал per-user.
