...
2. Если их нет (папка пустая / не та ветка), выполни:
     git fetch --all
     git checkout claude/advault-e11-polish-launch-drvjw3   # актуальная база: E0…E11 (весь MVP)
...
ЦЕЛЬ СЕССИИ — весь MVP (E0…E11) ГОТОВ по коду (веха M5). Дальше — релизные операции
и/или пост-MVP (E12+). См. готовый промт ниже. Кратко:
  - M5 Release (операции, `docs/17`): деплой (secrets/`migrate deploy`/CSP web-SPA),
    бэкапы+восстановление, мониторинг/алерты; бизнес-подтверждение провайдера/хостинга;
    финальная юр-вычитка ToS/Privacy/Refund.
  - Оставшиеся долги (не блокеры): аллокация discount при частичном возврате (E10),
    ...
    нагрузочное тестирование выдачи/оплаты.
...
## ✅ ГОТОВЫЙ ПРОМТ — Сессия M5 Release / пост-MVP (E12+)

> Весь MVP (E0…E11) готов по коду. Скопируй блок ниже в новую сессию и выбери трек:
> релизные операции или конкретную фичу E12+.
...
E10 гарантии/замены/возвраты · E11 полировка/безопасность/запуск (security-заголовки+CSP,
BullMQ-уведомления, отзывы/рейтинг, warm-rework↔claim статус `reworking`, юр-страницы,
E2E Playwright). Веха M5 достигнута по коду; чек-лист запуска docs/09 закрыт (кроме
...
(верхняя запись «E11» — что сделано и долги), docs/16-development-plan.md (§E12+ и вехи),
docs/17-operations-runbook.md (прод-операции), docs/backend/{prisma-schema,openapi}.md.
...
ЦЕЛЬ СЕССИИ — ВЫБЕРИ ТРЕК (уточни у пользователя в режиме asking, если не задано):
  ...
  C) Пост-MVP E12+ (оценка отдельно): интеграции Octo Browser API + провайдер прокси
     ...
     расширенная аналитика/BI, мультивендорность.
